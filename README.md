# AbortCommand
This simple aggressor script adds a context-menu option for beacons in CobaltStrike. The option "Abort Command" will clear the selected beacon's command queue.
...
# Installation
Git clone the repo or download just the AbortCommand.cna file. From CobaltStrike, navigate to the "Cobalt Strike" -> "Script Manager". Then use the "Load" button and select the aggressor script. The context menu for beacons will now include the "Abort Command" capability.
